<template>
  <Form ref="formItem" :model="formItem" :label-width="80">
    <Card dis-hover class="margin-top-20 large-card">
      ...
      <Row>
        <Col span="11">
        <Form-item label="订单单位列表">
          <Select v-model="formItem.cur_prod" placeholder="预览订单单位">
        ...
        <Form-item label="增加单位" prop="spec">
          <Input v-model="formItem.prod_unit" placeholder="请输入单位名称"></Input>
        </Form-item>
        <Form-item v-show="formItem.prod_unit.trim()!=''">
...
export default {
  ...
  methods: {
    handleSubmit: async function() {
      let params = {
        tbl: 0,
        tblname: "set_prod_unit_short",
        utf2gbk: ["prod_unit"],
        prod_unit: this.formItem.prod_unit
      };
